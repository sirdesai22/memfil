...
export interface UploadOptions {
  output?: string   // optional file to write the resulting CID to
}
...
export async function uploadCommand(filePath: string, options: UploadOptions): Promise<void> {
  ...
  let uploadResult: { pieceCid: string; size: number }

  try {
    const result = await synapse.storage.upload(fileBytes)
    let pieceCidString = String(result.pieceCid)
    const pc = result.pieceCid as unknown
    if (typeof pc === 'object' && pc !== null && '/' in pc && typeof (pc as { '/': string })['/'] === 'string') {
      pieceCidString = (pc as { '/': string })['/']
    }
