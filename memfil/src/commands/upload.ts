...
export async function uploadCommand(
  ...
  let carResult: Awaited<ReturnType<typeof createCarFromPath>>
  ...
  try {
    ...
    if (readiness.status === 'blocked') {
      ...
      if (v?.errorMessage) console.error(chalk.dim(`  • ${v.errorMessage}`))
      if (v?.helpMessage) console.error(chalk.dim(`  • ${v.helpMessage}`))
    ...
    try {
      await cleanupFilecoin()
      if (carResult?.carPath) await cleanupTempCar(carResult.carPath, log)
    } catch {
      /* ignore */
    }
    process.exit(1)
